...
public class ASMUtils {
    ...
    static {
        ...
        // properties. These exist for the CLI / research workflow that runs
        // against pre-collected dataset JARs and the developer's local Maven
        // repository; an installed IntelliJ plugin doesn't supply them, so
        // either being missing or pointing at a non-existent path is treated
        // as "no auxiliary JARs available". The two helpers that read from
        // {@code urlCL} ({@link #loadMethodsToClass} and {@link #isOverride})
        // already swallow {@code Exception | Error} and degrade gracefully
        // when a class is not found, so an empty classloader simply means
        // less inferred information about parent classes — never a crash.
        List<URL> urlList = new ArrayList<>();
        try {
            String mavenRepo = AppProperties.getString("mavenRepo", "");
            String datasetRoot = AppProperties.getString("datasetRoot", "");
            for (String pathRoot : new String[]{mavenRepo, datasetRoot}) {
                if (pathRoot == null || pathRoot.isEmpty()) continue;
                if (!new File(pathRoot).isDirectory()) {
                    logger.warn("Configured path '{}' does not exist; skipping.", pathRoot);
                    continue;
                }
                try {
                    for (String jar : FileUtils.getAllJarFiles(pathRoot)) {
                        urlList.add(new File(jar).toURI().toURL());
                    }
                } catch (RuntimeException scanFailure) {
                    logger.warn("Failed to scan '{}' for jars: {}", pathRoot, scanFailure.getMessage());
                }
            }
        } catch (Throwable t) {
            // Defensive: an empty classloader is always a safe fallback.
            logger.warn("Failed to initialise auxiliary classloader; continuing with empty path: {}",
                    t.getMessage());
        }
