...
/**
 * NSGA-II based refactoring engine. Uses the same problem formulation and
 * operators as {@link NSGAIIIRefactoringEngine} but with jMetal's NSGA-II
 * algorithm instead of NSGA-III.
 */
public class NSGAIIRefactoringEngine implements RefactoringEngine {
    @Override
    public RefactoringResult run(JavaProject project, RefactoringConfig refactoringConfig) {
        ...
        Algorithm<List<IntegerSolution>> algorithm =
                new NSGAIIBuilder<>(problem, crossover, mutation, refactoringConfig.getPopulationSize())
                        .setMaxEvaluations(refactoringConfig.getMaxIterations() * refactoringConfig.getPopulationSize())
